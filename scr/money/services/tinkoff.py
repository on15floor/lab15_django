...
class TinkoffAPI:
    ...
    def __init__(self, api_secret: str, broker_account_id: str):
        ...
        self.usd_currency = self.get_usd_currency()
    ...
    def get_portfolio_positions(self):
        """Получаем портфель вида: |Тикер|Имя|Валюта|Средняя цена|Кол-во|Возможная прибыль"""
...
def localize(d: datetime) -> datetime:
    return timezone(settings.TIME_ZONE).localize(d)
...
def build_collection(broker_account_id: str):
    """Собираем портфель и статистику в коллекцию для передачи в шаблон"""
    t = TinkoffAPI(settings.TINKOFF_SECRET_KEY, broker_account_id)
